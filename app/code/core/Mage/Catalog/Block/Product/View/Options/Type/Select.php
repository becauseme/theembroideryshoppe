    ...
    extends Mage_Catalog_Block_Product_View_Options_Abstract
{
      ...
      $selectc="radio";
            ...
            switch ($selectc) {
                case Mage_Catalog_Model_Product_Option::OPTION_TYPE_RADIO:
                    ...
                    $class = 'radio';
                    if (!$_option->getIsRequire()) {
                        $selectHtml .= '<li><input type="radio" id="options_'.$_option->getId().'" class="'.$class.' product-custom-option" name="options['.$_option->getId().']" onclick="opConfig.reloadPrice()" value="" checked="checked" /><span class="label"><label for="options_'.$_option->getId().'">' . $this->__('None') . '<img src="/media/nitin.jpg"></label></span></li>';
                    }
                    break;
                case Mage_Catalog_Model_Product_Option::OPTION_TYPE_CHECKBOX:
                    $type = 'checkbox';
            ...
            $count = 1;
            foreach ($_option->getValues() as $_value) {
				$imgf=str_replace(" ","",$_value->getTitle());
				$imgnewname=explode("_",$_value->getTitle());
                $count++;
                $priceStr = $this->_formatPrice(array(
                    'is_percent' => ($_value->getPriceType() == 'percent') ? true : false,
                    'pricing_value' => $_value->getPrice(true)
	 ...
	 public function getSizeHtml()
    {
      ...
      $selectc="checkbox";
            ...
            $selectHtml = '<ul id="options-'.$_option->getId().'-list" class="options-list12 option-list_cus">';
            ...
            foreach ($_option->getValues() as $_value) {
                ...
                $selectHtml .='<li>' .
                               '' .
                               '<div class="label sizelabel"><span><label for="options_'.$_option->getId().'_'.$count.'">'.$_value->getTitle().'<span><span  class="sizep"> '.$priceStr.'</span></div><br><input type="'.$type.'" class="'.$class.' '.$require.' product-custom-option"  name="options[size]['.$_option->getId().']['.$_value->getOptionTypeId().']'.$arraySign.'" id="options_'.$_option->getId().'_'.$count.'" value="" size="5" />';
                if ($_option->getIsRequire()) {
                  }
                $selectHtml .= '</li>';
            }
